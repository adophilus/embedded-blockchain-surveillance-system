...
import { Camera, Shield, Users, Activity, LogOut, Upload, Eye, Search, Filter, AlertCircle, CheckCircle, Clock, Database, BarChart, HardHat } from 'lucide-react';
...
const Login = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // In a real app, you would uncomment the actual apiCall.
      // const response = await apiCall('/auth/sign-in/email', {
      //   method: 'POST',
      ...
      if (email === 'test@police.gov' && password === 'password') {
        const mockUser = { full_name: 'Officer Alex Murphy', role: 'Chief Security Officer', id: 'user-1' };
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Blockchain-Powered Security</p>
          </div>
        </div>
...
const SessionEventModal = ({ selectedSession, events, loadingEvents, onClose }) => {
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          {loadingEvents ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {events.map((event) => (
                <div key={event.id} className="bg-slate-800 rounded-lg overflow-hidden border border-slate-700">
                  {event.ipfs_url ? (
                    <img
                      src={event.ipfs_url}
                  ...
                  <div className="p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs text-slate-500">Device: {event.device_id}</span>
                      {event.detected ? (
                    ...
                    {event.ipfs_cid && (
                      <p className="text-xs text-blue-400 mt-2 truncate" title={event.ipfs_cid}>
                        IPFS: {event.ipfs_cid}
                      </p>
                    )}
                  </div>
...
const SurveillanceTab = ({ sessions }) => {
  ...
  const loadSessionEvents = async (sessionId, session) => {
    ...
    try {
      ...
      const mockEvents = [
        { id: 1, device_id: 'CAM-A1', timestamp: Date.now() / 1000 - 3600, detected: false, ipfs_cid: 'QmHash123', ipfs_url: 'https://via.placeholder.com/300x200?text=Clear+Event' },
        { id: 2, device_id: 'CAM-B2', timestamp: Date.now() / 1000 - 1800, detected: true, ipfs_cid: 'QmHash456', ipfs_url: 'https://via.placeholder.com/300x200/ff0000/ffffff?text=ALERT%3A+Detection' },
        { id: 3, device_id: 'CAM-C3', timestamp: Date.now() / 1000 - 600, detected: false, ipfs_cid: 'QmHash789', ipfs_url: 'https://via.placeholder.com/300x200?text=Clear+Event' },
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(session.status)} text-white`}>
                  {session.status}
                </span>
              </div>
            </div>
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 space-y-4">
              ...
              {criminal.aliases?.length > 0 && (
                <div>
                  <label className="text-sm text-slate-400">Known Aliases</label>
              ...
              <div>
                ...
                <p className="text-white">{formatDateTime(criminal.created_at)}</p>
              </div>

              {criminal.mugshot?.id && (
                <div>
                  <label className="text-sm text-slate-400">IPFS Hash</label>
                  <p className="text-blue-400 text-sm break-all">{criminal.mugshot.id}</p>
                </div>
              )}
            </div>
          </div>
...
const CriminalsTab = ({ criminals, setCriminals }) => {
  ...
  const mockCriminalsData = [
    { id: 'crim-1', name: 'Al Capone', aliases: ['Scarface'], offense: 'Racketeering, Tax Evasion', created_at: Date.now() / 1000 - 86400 * 5, mugshot: { id: 'QmCaponeIPFS', url: 'https://via.placeholder.com/192x192/8b0000/ffffff?text=Al+Capone' } },
    { id: 'crim-2', name: 'Bonnie Parker', aliases: ['Bonniewho'], offense: 'Bank Robbery, Murder', created_at: Date.now() / 1000 - 86400 * 10, mugshot: { id: 'QmBonnieIPFS', url: 'https://via.placeholder.com/192x192/8b0000/ffffff?text=Bonnie+Parker' } },
    { id: 'crim-3', name: 'John Dillinger', aliases: ['Jack Rabbit'], offense: 'Bank Robbery, Escape', created_at: Date.now() / 1000 - 86400 * 2, mugshot: { id: 'QmDillingerIPFS', url: 'https://via.placeholder.com/192x192/8b0000/ffffff?text=John+Dillinger' } },
    { id: 'crim-4', name: 'Jesse James', aliases: null, offense: 'Train Robbery, Murder', created_at: Date.now() / 1000 - 86400 * 30, mugshot: null },
  ];
...
const UploadTab = ({ onUploadSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.aliases) {
        const aliasesArray = formData.aliases.split(',').map(a => a.trim()).filter(a => a !== '');
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-2xl font-bold text-white">Upload Criminal Profile</h2>
      </div>
...
const MetricsTab = () => {
  const metrics = [
    { title: 'Total Surveillance Sessions', value: '45', icon: Camera, color: 'text-blue-400', bg: 'bg-blue-900/50' },
    { title: 'Criminal Records on Chain', value: '8,421', icon: Users, color: 'text-red-400', bg: 'bg-red-900/50' },
    { title: 'Detection Alerts (Past 24h)', value: '128', icon: AlertCircle, color: 'text-yellow-400', bg: 'bg-yellow-900/50' },
    { title: 'Successful IPFS Uploads', value: '99.8%', icon: CheckCircle, color: 'text-green-400', bg: 'bg-green-900/50' },
  ];
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-2xl font-bold text-white">System Metrics & Analytics</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {metrics.map((metric, index) => {
          ...
          return (
            <div key={index} className={`p-6 rounded-xl border border-slate-800 ${metric.bg}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
          ...
          <ul className="space-y-3">
            {['Bank Robbery', 'Assault', 'Racketeering', 'Vandalism', 'Homicide'].map((offense, index) => (
              <li key={offense} className="flex items-center justify-between p-3 bg-slate-800 rounded-lg">
                <span className="text-slate-300">{index + 1}. {offense}</span>
                <span className="text-sm font-bold text-red-400">{Math.floor(Math.random() * 50 + 10)} Incidents</span>
              </li>
            ))}
          </ul>
        </div>
...
const MainContent = ({ activeTab, sessions, criminals, setCriminals, setActiveTab }) => {
...
const App = () => {
  ...
  // MOCK SESSION DATA
  const mockSessions = [
    { id: 'sess-101', title: 'Downtown Patrol - Sector 4', description: 'Routine surveillance focusing on known organized crime areas.', status: 'ACTIVE', start_timestamp: Date.now() / 1000 - 3600 * 2, end_timestamp: Date.now() / 1000 + 3600 * 10 },
    { id: 'sess-102', title: 'Bank District Monitoring', description: 'High-security monitoring following recent robbery attempt.', status: 'COMPLETED', start_timestamp: Date.now() / 1000 - 3600 * 48, end_timestamp: Date.now() / 1000 - 3600 * 24 },
    { id: 'sess-103', title: 'Industrial Zone Check', description: 'Scheduled deep-scan for infrastructure security.', status: 'UPCOMING', start_timestamp: Date.now() / 1000 + 3600 * 72, end_timestamp: Date.now() / 1000 + 3600 * 96 },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-900">
      ...
      <footer className="bg-slate-900 border-t border-slate-800 py-4 text-center text-sm text-slate-500">
        &copy; {new Date().getFullYear()} Blockchain Surveillance System. All rights reserved.
      </footer>
    </div>
  );
};
