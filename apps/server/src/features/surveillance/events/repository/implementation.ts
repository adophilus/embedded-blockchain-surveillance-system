...
import type {
  ...
  SurveillanceEventRepositoryError,
} from "./interface";
import type { KyselyClient } from "@/features/database/kysely";
import type { Logger } from "@/features/logger";

export class KyselySurveillanceEventRepository implements SurveillanceEventRepository {
  constructor(
  ...
  public async list(
    ...
    try {
      ...
      return Result.ok(events.map(event => ({
        ...event,
        detected: event.detected === 1,
      })));
    } catch (error) {
